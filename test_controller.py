...
@pytest.fixture(name="controller_fixture")
def make_controller_fixture():
    """Create a controller fixture with mocked environment and model."""
    view = MagicMock()
    view.thresholds = {"nudge_strength": 0.5, "nudge_display_ttl": 20}
...
def test_nudge_right_updates_state(controller_fixture):
    """
    ...
    """
    mock_keys = [0] * 512
    with patch("pygame.key.get_pressed", return_value=mock_keys), patch(
        "pygame.K_LEFT", 0
    ), patch("pygame.K_RIGHT", 1):

        mock_keys[1] = 1
        controller_fixture.nudges(nudge_ttl=0, last_nudge_dir=0)

        assert controller_fixture.env.unwrapped.state[3] == 0.5


def test_nudge_returns_updated_ui_metadata(controller_fixture):
    """
    ...
    with patch("pygame.key.get_pressed", return_value=mock_keys), patch(
        ...
        mock_keys[0] = 1
        _, ttl, direction = controller_fixture.nudges(nudge_ttl=0, last_nudge_dir=0)

        assert ttl == 20
        assert direction == -1


def test_setup_logic_creates_new_model_if_missing(controller_fixture):
    """
    ...
    """
    with patch("controller.CustomCartPole"), patch("controller.TimeLimit"), patch(
        "os.path.exists", return_value=False
    ), patch("controller.DQN") as mock_dqn:

        needs_train = controller_fixture.setup_env_and_model(1.0, 0.1)

        assert needs_train is True
        mock_dqn.assert_called_once()


def test_setup_logic_loads_existing_model(controller_fixture):
    """
...
def test_observation_trig_conversion_accuracy(controller_fixture):
    """
    ...
    """
    controller_fixture.env.unwrapped.state = np.array(
        [0.0, 0.0, np.pi / 2, 0.0], dtype=np.float32
    )
    ...
    with patch("pygame.key.get_pressed", return_value=mock_keys), patch(
        ...
        controller_fixture.nudges(0, 0)

        args, _ = controller_fixture.model.predict.call_args
        obs = args[0]
...
def test_simulation_event_handling_restart(controller_fixture):
    """
    ...
    with patch("pygame.event.get", return_value=[mock_event]), patch(
        "pygame.key.get_pressed", return_value=[0] * 512
    ), patch("controller.pygame.K_r", 114), patch(
        "controller.pygame.surfarray.make_surface"
    ), patch(
        "controller.pygame.transform.scale"
    ), patch(
        "controller.pygame.display.flip"
    ):

        result = controller_fixture.run_simulation()

        assert result == "RESTART"


def test_simulation_event_handling_quit(controller_fixture):
    """
    ...
    with patch("pygame.event.get", return_value=[mock_event]), patch(
        "pygame.key.get_pressed", return_value=[0] * 512
    ), patch("controller.pygame.K_ESCAPE", 27), patch(
        "controller.pygame.surfarray.make_surface"
    ), patch(
        "controller.pygame.transform.scale"
    ), patch(
